...
import { feedbackModal } from "../models/feedback.js";
...
export const userCount = async (req, res) => {
  ...
  let Allfeedbacks = [];
  for (const userModel of user) {
    // Find the corresponding feedback for each user in the "feedbacks" collection
    const getfeedback = await feedbackModal.findOne({
      userId: userModel._doc._id,
    });

    if (getfeedback) {
      // Do something with the feedback
      console.log(
        `User: ${userModel.firstName}, Feedback: ${getfeedback.feedback}`
      );
      Allfeedbacks.push(getfeedback);
    } else {
      console.log(`User: ${userModel.name}, No feedback found.`);
    }
  }
...
export const handleUserSearch = async (req, res) => {
  ...
  if (response) {
    ...
    //Now also got the Feed back Of all serached Users Below

    let Allfeedbacks = [];
    for (const userModel of response) {
      // Find the corresponding feedback for each user in the "feedbacks" collection
      const getfeedback = await feedbackModal.findOne({
        userId: userModel._doc._id,
      });
